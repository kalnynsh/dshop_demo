...
class YandexMarket
{
    ...
    public function __construct(
        ...
        DeliveryReadRepository $delivaries
    ) {
        $this->shop = $shop;
    ...
    public function generate(callable $productUrlGenerator): string
    {
        ...
        $writer->endElement(); // end currencies
        ...
        foreach ($this->categories->getAll() as $category) {
            ...
            if (($parent = $category->parent)
                ...
                $writer->writeAttribute('parnetId', $parent->id);
            }
        ...
        $allDelivaries = $this->delivaries->getAll();
        ...
        foreach ($this->products->getAllIterator() as $product) {
            ...
            $available = array_filter($allDelivaries, function (Delivery $method) use ($product) {
            ...
            if (!empty($available)) {
                ...
                $writer->writeElement(
                    'local_delivery_cost',
                    \max(array_map(function (Delivery $method) {
                        return $method->cost;
                    }, $available))
                );
            }
