...
class Product extends ActiveRecord
{
    ...
    public function canBeCheckout($modificationId, $dQuantity): bool
    {
        if ($modificationId) {
            return $dQuantity <= $this->getModification($modificationId)->quantity;
        }

        return $dQuantity <= $this->quantity;
    }
    ...
    public function checkout($modificationId, $quantity): void
    {
        ...
        if ($quantity > $this->quantity) {
            throw new \DomainException('Only ' . $this->quantity . ' items avialable.');
        }
    ...
    public function changeQuantity($quantity): void
    {
        if ($this->modifications) {
            throw new \DomainException('You must change modifications quantity.');
        }
    ...
    public function setValue($id, $newValue): void
    {
        ...
        $currentValues[] = Value::create($id, $newValue);
        $this->values = $currentValues;
    }
    ...
    public function removeModification($id): void
    {
        ...
        foreach ($modifications as $idx => $modification) {
            if ($modification->isIdEqualTo($id)) {
                unset($modifications[$idx]);
                $this->updateModifications($modifications);
    ...
    public function updateModifications(array $modifications): void
    {
        ...
        $totalMQuantity = array_sum(
            array_map(function (Modification $modification) {
                return $modification->quantity;
            }, $this->modifications)
        );

        $this->setQuantity($totalMQuantity);
    }
    ...
    public function afterSave($insert, $changedAttributes): void
    {
        ...
        if (array_key_exists('mainPhoto', $related)) {
            $this->updateAttributes([
                'main_photo_id' => $related['mainPhoto'] ? $related['mainPhoto']->id : null
            ]);
        }
    }
